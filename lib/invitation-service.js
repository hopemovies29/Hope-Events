...
function buildOwnerEventPayload(eventRecord, invitationRecords) {
  return {
    ...
    accessKey: eventRecord.accessKey || "",
    coupleNames: eventRecord.coupleNames || "",
    dateLabel: eventRecord.dateLabel || formatDateLabel(eventRecord.dateIso),
    venueName: eventRecord.venueName || "",
    venueAddress: eventRecord.venueAddress || "",
    ...
    invitations: invitationRecords
      ...
      .map(function (item) {
        return {
          ...
          tableName: item.tableName || "Sans table",
          tableSlug: item.tableSlug || "",
          seats: item.seats || 1,
          invitationUrl: `/invitation?token=${encodeURIComponent(item.token)}`
        };
      })
      .sort(function (a, b) {
...
async function getEventSpaceByKey(key) {
  ...
  if (!db) {
    const matchesDemoEvent =
      demoEvent.accessKey === normalizedKey ||
      demoEvent.slug === normalizedKey;
  ...
  if (!eventQuery.empty) {
    ...
    const eventBySlug = await db.collection(eventsCollection).doc(normalizedKey).get();
