    ...
    invitations: invitationRecords
      ...
      .map(function (item) {
        ...
        return {
          ...
          invitationPagePath: cleanText(item.invitationPagePath) || "",
          publicInvitationPath: cleanText(item.publicInvitationPath) || "",
          publicQrPagePath: cleanText(item.publicQrPagePath) || "",
          seats: item.seats || 1,
          isAdminCreated: item.createdFrom === "client-space",
...
async function getInvitationByToken(token) {
  ...
  // New invitations added from the client space share the existing event
  // visual and details, without requiring a duplicate Firebase event record.
  if (eventId === christianSephoraGuests.event.id) {
    return {
      mode: "firebase",
      data: buildInvitationPayload(christianSephoraGuests.event, {
        ...invitationRecord,
        token
      })
    };
...
async function getGeneratedChristianSephoraSpace() {
  const staticInvitations = Object.values(christianSephoraGuests.invitations).map(function (invitation) {
    return {
      ...invitation,
      ...(christianSephoraGuests.routes[invitation.token] || {})
    };
  });
  const db = getFirestore();
  ...
  if (!db) {
    return {
      mode: "server",
      data: buildOwnerEventPayload(christianSephoraGuests.event, staticInvitations)
    };
  }
  ...
  const dynamicSnap = await db
    .collection(invitationsCollection)
    .where("eventId", "==", christianSephoraGuests.event.id)
    .get();
  const dynamicInvitations = dynamicSnap.docs
    .map(function (doc) {
      return { token: doc.id, ...doc.data() };
    })
    .filter(function (invitation) {
      return !christianSephoraGuests.invitations[invitation.token];
    });

  return {
    mode: "firebase",
    data: buildOwnerEventPayload(
      christianSephoraGuests.event,
      staticInvitations.concat(dynamicInvitations)
    )
  };
}
...
async function getEventSpaceByKey(key) {
  ...
  if (isGeneratedEvent) {
    return getGeneratedChristianSephoraSpace();
  }
...
async function createChristianSephoraInvitation(payload) {
  const guestName = cleanText(payload.guestName).replace(/\s+/g, " ");
  const tableName = cleanText(payload.tableName).replace(/\s+/g, " ");

  if (!guestName || !tableName) {
    throw new Error("Le nom de l'invite et la table sont obligatoires.");
  }

  if (guestName.length > 120 || tableName.length > 80) {
    throw new Error("Le nom de l'invite ou de la table est trop long.");
  }

  const db = getFirestore();
  if (!db) {
    throw new Error("Firebase n'est pas configure. Ajoutez les variables Firebase dans Vercel.");
  }

  const token = `cs-${slugify(tableName)}-${slugify(guestName)}-${crypto.randomBytes(4).toString("hex")}`;
  const basePath = "/couple-christian-sephora";
  const encodedToken = encodeURIComponent(token);
  const invitationPath = `${basePath}/invitation?token=${encodedToken}`;
  const qrPath = `${basePath}/carte-qr?token=${encodedToken}`;
  const record = {
    eventId: christianSephoraGuests.event.id,
    guestName,
    tableName,
    tableSlug: slugify(tableName),
    salutation: guestName.toLowerCase().startsWith("couple") ? "Cher invite" : "Chere invite",
    seats: guestName.toLowerCase().startsWith("couple") ? 2 : 1,
    isActive: true,
    invitationPagePath: "./invitation.html",
    qrPagePath: "./carte-qr.html",
    publicInvitationPath: invitationPath,
    publicQrPagePath: qrPath,
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
    createdFrom: "client-space"
  };

  await db.collection(getCollectionName("FIREBASE_INVITATIONS_COLLECTION", "invitations")).doc(token).set(record);

  return {
    token,
    ...record,
    invitationUrl: invitationPath,
    qrUrl: qrPath
  };
}

async function getEditableChristianSephoraInvitation(token) {
  const db = getFirestore();
  if (!db) {
    throw new Error("Firebase n'est pas configure.");
  }

  const reference = db.collection(getCollectionName("FIREBASE_INVITATIONS_COLLECTION", "invitations")).doc(token);
  const snapshot = await reference.get();
  const invitation = snapshot.exists ? snapshot.data() : null;

  if (!invitation || invitation.eventId !== christianSephoraGuests.event.id || invitation.createdFrom !== "client-space") {
    throw new Error("Seules les invitations ajoutees depuis l'admin peuvent etre modifiees.");
  }

  return { reference, invitation };
}

async function updateChristianSephoraInvitation(payload) {
  const token = cleanText(payload.token);
  const guestName = cleanText(payload.guestName).replace(/\s+/g, " ");
  const tableName = cleanText(payload.tableName).replace(/\s+/g, " ");

  if (!token || !guestName || !tableName || guestName.length > 120 || tableName.length > 80) {
    throw new Error("Le nom de l'invite et la table sont obligatoires.");
  }

  const { reference, invitation } = await getEditableChristianSephoraInvitation(token);
  const updated = {
    guestName,
    tableName,
    tableSlug: slugify(tableName),
  ...
  return {
    ...
    ...invitation,
    ...updated,
    invitationUrl: invitation.publicInvitationPath,
    qrUrl: invitation.publicQrPagePath
  };
}

async function deleteChristianSephoraInvitation(token) {
  const { reference } = await getEditableChristianSephoraInvitation(cleanText(token));
  await reference.set({ isActive: false, deletedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
  return { token: cleanText(token) };
}
